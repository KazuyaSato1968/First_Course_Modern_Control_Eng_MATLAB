...
%図9.1のプロット
figure(1) %図のウィンドウを開く
plot(t,y1(:,1),'-b',t,y1(:,2),'-r'); %x_1とx_2を抽出してプロット
xlim([0 10]); %横軸（時間軸）の範囲の指定
ylim([-1.0 1.5]); %縦軸の範囲の設定
grid; %罫線を表示
...
xlim([0 10]); %横軸（時間軸）の範囲の指定
ylim([0.0 1.5]); %縦軸の範囲の設定
grid; %罫線を表示
xlabel('time t[s]'); %横軸のラベル表示
ylabel('x_1(t)'); %縦軸のラベル表示
legend('x_1(t) of Ex.9.1','x_1(t) of Ex.9.2'); %凡例の表示
...
legend('x_2(t) of Ex.9.1','x_2(t) of Ex.9.1'); %凡例の表示