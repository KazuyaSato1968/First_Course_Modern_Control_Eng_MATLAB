...
%配置するオブザーバの極を与える
op = [-5 + 6i;-5 - 6i]; %-5 + 6i, -5 - 6iをオブザーバの極とする
%
...
%図11.7(b)のプロット(図11.4(a)の再現)
%配置するオブザーバの極を与える
op2 = [-4;-5]; %-4, -5をオブザーバの極とする

%オブザーバゲインを求める．双対システムA^Tとc^Tの状態フィードバックゲインを求め，転置を取るとオブザーバゲインとなる
h2 = acker(A',c',op2)' %双対システムで極配置し，転置を求める

%Ah = A-hcとオブザーバの極の計算と表示
Ah2 = A - h2*c; %Ah = A - h*cの計算
disp('A-hcの固有値');
eig(A - h2 * c) %オブザーバの極を計算し表示
...
sys = ss(A, b, cc, dd); %システムの状態空間表現
e_sys2 = ss(Ah2, b, cc, dd); %誤差システムの状態空間表現

e2 = initial(e_sys2,x0_ob - x0,t); %f2の場合の初期値応答
%オブザーバの状態を求める
h_x2 = e2 + y; %e(t) = hat{x}(t) - x(t)よりhat{x}を求める

figure(4) %図のウィンドウを開く
plot(t,y(:,1),t,y(:,2),t,h_x2(:,1),t,h_x2(:,2)); %各状態を抽出してプロット
xlim([0 4]); %横軸（時間軸）の範囲の指定
ylim([-1.0 3.0]); %縦軸の範囲の設定
grid; %罫線を表示
xlabel('time t[s]'); %横軸のラベル表示
