...
%
h_x_1 = ye(:,3) + ye(:,1); %併合システムの推定値の計算
h_x_2 = ye(:,4) + ye(:,2); %併合システムの推定値の計算

%図12.3(a)のプロット
figure(1) %図のウィンドウを開く
%plot(t,y(:,1),t,y(:,2),t,ye(:,1),t,ye(:,2)); %各状態を抽出してプロット
plot(t,ye(:,1),t,ye(:,2),t,y(:,1),t,y(:,2)); %各状態を抽出してプロット
...
%図12.3(b)のプロット
figure(2) %図のウィンドウを開く
plot(t,ye(:,1),t,ye(:,2),t,h_x_1,t,h_x_2); %各状態を抽出してプロット
xlim([0 5]); %横軸（時間軸）の範囲の指定
ylim([-2.0 3]); %縦軸の範囲の設定
grid; %罫線を表示
xlabel('time t[s]'); %横軸のラベル表示
